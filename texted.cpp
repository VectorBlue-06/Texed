...
std::string editLine(const std::string& initial = "") {
    ...
    while (true) {
        ...
        // Arrow keys
            ...
            else if (key == 77 && cursor < (int)text.size()) cursor++;  // Right
        }
...
//TODO aligned bullets
//FIX e without number crashes the program
//FIX edit mode still showing editing line after pressing enter
...
int main(){
    ...
    {   
        ...
        if(command_mode == true){
            ...
            if(cmd=="q") {should_run = false; break;}
            if(cmd=="n") command_mode = false;
